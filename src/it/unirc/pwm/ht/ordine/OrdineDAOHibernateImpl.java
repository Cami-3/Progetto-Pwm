...
public class OrdineDAOHibernateImpl implements OrdineDAO{
	...
	public List<Ordine> getOrdiniByCliente(Cliente c) {
		...
		try {
			list = (List<Ordine>) session.createQuery("from Ordine o where o.cliente= :cliente").setParameter("cliente",c).list();
			transaction.commit();
		} catch (HibernateException e) {
			transaction.rollback();
			return null;
	...
	public List<Ordine> getAllOrdini() {
		...
		List<Ordine> list=null; 
		try {
			list = (List<Ordine>) session.createQuery("from Ordine").list();
			transaction.commit();
		} catch (HibernateException e) {
			transaction.rollback();
			return null;
